...
switch($Choice)
{
  0 { exit }
  1 {   
        Write-Host "FAILOVER to EUS2 - Unegister EUS Region Workspace" -ForegroundColor Yellow -BackgroundColor DarkGreen
        ...
        write-host "raw CSV " $CSV -ForegroundColor Yellow -BackgroundColor DarkGreen
        ForEach ($Item in $CSV)
        {
            $SessHost = $Item.Name
            $CurSessID = $Item.Name
            write-host $SessHost -ForegroundColor Yellow -BackgroundColor DarkGreen
            ...
            else
            {
                ...
                write-host $SessionHostName -ForegroundColor Yellow -BackgroundColor DarkGreen
                $SessionId = Format-UserSessionId($CurSessID)
                write-host $SessionId -ForegroundColor Yellow -BackgroundColor DarkGreen
                $Error.Clear()
                IF ($LogOffOrRemove -eq "Remove")
                {
                    Write-Host "Forcing User session Log Off" -ForegroundColor Yellow -BackgroundColor DarkGreen
                ...
                {
                    Write-Host "Disconnect User Session " -ForegroundColor Yellow -BackgroundColor DarkGreen
    ...
    write-host "unregistering app group from workspace: " $WorkspaceNameEUS -ForegroundColor Yellow -BackgroundColor DarkGreen
    Unregister-AzWvdApplicationGroup -ResourceGroupName $ResourceGroupNameEUS `
        ...
        write-host "registering app group to workspace: " $WorkspaceNameEUS2 -ForegroundColor Yellow -BackgroundColor DarkGreen
    ...
    write-host "FAILOVER COMPLETE" -ForegroundColor Yellow -BackgroundColor DarkGreen
    }
    

    }  
  2 { Write-Host "FAILOVER to EUS - Unegister EUS2 Region Workspace" -ForegroundColor Yellow -BackgroundColor DarkGreen
    $SubscriptionIdEUS = "e446e661-f7b9-4343-b93a-1e719e986bd0"
    $ResourceGroupNameEUS = "rg-avd-pool-1"
    $WorkspaceNameEUS = "WS-Pooled-Desktops"
    $HostPoolEUS = "HP-Pooled-Desktops"
    ...
    ForEach ($Item in $CSV)
    {
        ...
        else
        {
            ...
            IF ($LogOffOrRemove -eq "Remove")
            {
                Write-Host "Forcing User Log Off" -ForegroundColor Yellow -BackgroundColor DarkGreen
            ...
            IF ($Error.Count -eq 0)
            {
                Write-Host "User Was Removed..." -ForegroundColor Yellow -BackgroundColor DarkGreen
            }
            else
            {
                Write-Host "ErrorRemovingUser:$Error"
    ...
    write-host "unregistering app group from workspace: " $WorkspaceNameEUS2 -ForegroundColor Yellow -BackgroundColor DarkGreen
    Unregister-AzWvdApplicationGroup -ResourceGroupName $ResourceGroupNameEUS2 `
    -WorkspaceName $WorkspaceNameEUS2 `
    -ApplicationGroupPath $ApplicationGroupPathEUS2    
    
    write-host "registering app group to workspace: " $WorkspaceNameEUS -ForegroundColor Yellow -BackgroundColor DarkGreen
